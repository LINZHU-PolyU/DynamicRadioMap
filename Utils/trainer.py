...
# ---------------------------------------------------------------------------
#  1. PERCEPTUAL LOSS — VGG-based, applied slice-wise to 3D volumes
# ---------------------------------------------------------------------------
class VGGFeatureExtractor(nn.Module):
    """Extract multi-scale features from VGG-16 for perceptual loss."""

    _LAYER_INDICES = (3, 8, 15, 22)

    def __init__(self):
        super().__init__()
        vgg = models.vgg16(weights=models.VGG16_Weights.DEFAULT)
        # Keep only the layers up to the last one we need
    ...
    def forward(self, x: torch.Tensor) -> list[torch.Tensor]:
        ...
        Args:
            x: (B, 3, H, W) image tensor in [0, 1]. If the input has 1 channel
               it is expanded to 3 by repeating.

        Returns:
            List of feature maps at each extraction layer.
        """
        if x.shape[1] == 1:
            x = x.expand(-1, 3, -1, -1)
...
def perceptual_loss_3d(
    pred: torch.Tensor,
    target: torch.Tensor,
    vgg: VGGFeatureExtractor,
    slice_dim: int = 2,
) -> torch.Tensor:
    """Compute perceptual loss on a 3D volume by averaging over 2-D slices."""
    # Flatten any leading (B, T, ...) into a single batch dimension
    orig_shape = pred.shape
    if pred.ndim == 5:                       # (B, T, D, H, W)
        B, T, D, H, W = orig_shape
        pred = pred.reshape(B * T, D, H, W)
        target = target.reshape(B * T, D, H, W)
    # Now pred/target are (N, D, H, W) where D=64, H=W=128

    N, D, H, W = pred.shape
    # Collect all slices into one big batch: (N*D, 1, H, W)
    pred_slices = pred.reshape(N * D, 1, H, W)
    target_slices = target.reshape(N * D, 1, H, W)

    # Compute features (the extractor handles 1→3 channel expansion)
    pred_feats = vgg(pred_slices)
    target_feats = vgg(target_slices)
...
#  2. LOSS COMPONENTS
# ---------------------------------------------------------------------------
def _loss_components(pred, target, lambda_temp: float, vgg: VGGFeatureExtractor = None, lambda_perc: float = 0.0):
    """Decompose the WavePropLoss into its parts (recon + temporal + perceptual)."""
    l_recon = F.mse_loss(pred, target)
    delta_pred = pred[:, 1:] - pred[:, :-1]
    delta_target = target[:, 1:] - target[:, :-1]
    l_temp = F.mse_loss(delta_pred, delta_target)

    combined = l_recon + lambda_temp * l_temp

    if vgg is not None and lambda_perc > 0.0:
        l_perc = perceptual_loss_3d(pred, target, vgg)
        combined = combined + lambda_perc * l_perc
    else:
        l_perc = torch.tensor(0.0, device=pred.device)

    return l_recon, l_temp, l_perc, combined


# ---------------------------------------------------------------------------
#  3. EPOCH LOOPS
# ---------------------------------------------------------------------------
def train_epoch(model, loader, optimizer, lambda_temp: float, device,
                grad_clip_norm: float = 1.0, vgg: VGGFeatureExtractor = None, lambda_perc: float = 0.0):
    """Run one training epoch."""
    model.train()
    tot, rec, tmp, prc, n_samples = 0.0, 0.0, 0.0, 0.0, 0
    pbar = tqdm(loader, desc="  Train", leave=False, ncols=120)
    for bld, traj, rm in pbar:
        bld = bld.to(device)
        traj = traj.to(device)
        rm = rm.to(device)
        ...
        optimizer.zero_grad()
        pred = model(bld, traj)
        l_rec, l_tmp, l_prc, loss = _loss_components(pred, rm, lambda_temp, vgg, lambda_perc)
        loss.backward()
        ...
        batch_size = bld.size(0)
        n_samples += batch_size
        tot += loss.item() * batch_size
        rec += l_rec.item() * batch_size
        tmp += l_tmp.item() * batch_size
        prc += l_prc.item() * batch_size
        pbar.set_postfix(
            loss=f"{tot / n_samples:.4f}",
            recon=f"{rec / n_samples:.4f}",
            temp=f"{tmp / n_samples:.4f}",
            perc=f"{prc / n_samples:.4f}",
        )

    return tot / n_samples, rec / n_samples, tmp / n_samples, prc / n_samples


@torch.no_grad()
def eval_epoch(model, loader, lambda_temp: float, device,
               vgg: VGGFeatureExtractor = None, lambda_perc: float = 0.0):
...
@torch.no_grad()
def test_epoch(model, loader, lambda_temp: float, device,
               vgg: VGGFeatureExtractor = None, lambda_perc: float = 0.0):
    ...
    for bld, traj, rm in tqdm(loader, desc="Testing", leave=False, ncols=120):
        bld = bld.to(device)
        traj = traj.to(device)
        rm = rm.to(device)
        pred = model(bld, traj)
...
_LOG_FIELDS = (
    "epoch",
    "train_total", "train_recon", "train_temp", "train_perc",
    "val_total", "val_recon", "val_temp", "val_perc",
    "lr", "elapsed_s",
)
...
def plot_training_curves(log_path: Path, out_dir: Path):
    ...
    ax.plot(epochs, d["train_temp"], color="steelblue", label="Train temp", linestyle=":")
    ax.plot(epochs, d["train_perc"], color="steelblue", label="Train perc", linestyle="-.")
    ax.plot(epochs, d["val_recon"], color="tomato", label="Val recon", linestyle="--")
    ax.plot(epochs, d["val_temp"], color="tomato", label="Val temp", linestyle="-.")
    ax.plot(epochs, d["val_perc"], color="tomato", label="Val perc", linestyle=":")
